...
export default function UpdatePasswordPage(){
    ...
    const updatePassword = async () => {
        try {
            ...
            console.log("error in (app/updatepassword/updatePasswordPage)", error)
            setError(true);
            setLoading(false);
        }
    }
    ...
    useEffect(() => {
        const verifyToken = async () => {
            try {
                await axios.get('/api/users/updatepassword', { params: { token, password } })
                setTokenVerified(true);
            } catch (error: any) {
                console.log('Error in (app/updatepassword/page/useEffect)', error)
                setError(true);
            }
        }
        if (token.length > 0) {
